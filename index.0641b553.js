...
const formatDate = (0, _dateFns.format)(today, "MM/dd/yyyy");
let feedback = [
    {
        id: 1,
        author: "Анна",
...
function renderFeedback() {
    ...
    <div class="button-container">
        <button id="next" onclick="nextReview()">Назад</button>
        <button id="prev" onclick="previousReview()">Вперед</button>
        </div>
        <div>
        <button id="random-btn" onclick="randomReview()">Случайный отзыв</button>
        </div>
    </div>`;
}
function previousReview() {
    if (feedback >= review.length) rev = 0;
    else rev = rev + 1;
    if (rev > review.length + 2) rev = 0;
    renderFeedback();
...
function nextReview() {
    ...
    if (rev < 0) rev = review.length + 2;
    renderFeedback();
    const feedbackJson = JSON.stringify(feedback);
    localStorage.setItem("feedback", feedbackJson);
}
function randomReview() {
    return rev[Math.floor(Math.random() * review.length)];
}
const feedbackJson = localStorage.getItem("feedback");
const savedfeedback = JSON.parse(feedbackJson);
if (feedbackJson) feedback = savedfeedback;
renderFeedback();
